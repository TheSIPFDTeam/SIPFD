...
typedef struct {
    fp_t k;         // k : integer scalar for computing the kernel R = P + [k]Q
    uint8_t b;       // b : P + [b*DEGREE^(EXPONENT_HALVES - 1) + k]Q if 0 <= b < DEGREE; [2k]P + Q if b = DEGREE.
} point_t;
...
void _h_(proj_t G, const proj_t P[3], const proj_t A2, const point_t g, const digit_t deg, const fp_t bound);
void _gn_(point_t *g, const fp2_t jinv, const digit_t NONCE, const digit_t deg, const ctx_mitm_t context);
void _fn_(point_t *y, fp2_t j, const point_t x, const ctx_mitm_t conf, const digit_t *S[2]);
