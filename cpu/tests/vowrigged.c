...
{
	printf("Vow-rigged only supported for montgomery\n");
	return 0;
}
...
int main (int argc, char **argv)
{
    ...
    memset(data, 0, 101);
    memset(entity, 0, 101);
    while ((opt = getopt(argc, argv, "hc:s:w:p")) != -1) {
        switch (opt) {
            case 'h':
                usage_vowgcs();
                return 0;
    ...
    if ((strcmp(entity, "Alice") == 0) || (strcmp(entity, "Alicia") == 0) || (strcmp(entity, "\0") == 0) )
        deg = 2;
    else
    {
        printf("Error: rigged vowgcs only works with deg=2 (Alice)\n");
        usage_vowgcs();
        return -1;
    }
    ...
    // Overwrite with the fixed instance
    proj_copy(context.E[0], E[0]);
    proj_copy(context.E[1], E[1]);
    proj_copy(context.notE[0], notE[0]);
    proj_copy(context.notE[1], notE[1]);
    proj_copy(context.A2[0], E[0]);
    proj_copy(context.A2[1], E[1]);
    proj_copy(context.BASIS[0][0], BASIS[0][0]);
    proj_copy(context.BASIS[0][1], BASIS[0][1]);
    proj_copy(context.BASIS[0][2], BASIS[0][2]);
    proj_copy(context.BASIS[1][0], BASIS[1][0]);
    proj_copy(context.BASIS[1][1], BASIS[1][1]);
    proj_copy(context.BASIS[1][2], BASIS[1][2]);
    ...
    (&ctx)->triplet_bytes = (2*((&context)->ebits_max+1) - (&ctx)->omegabits - (&ctx)->n + (&ctx)->trail_bits - 1)/8 + 1;
...
#pragma omp parallel shared(golden,finished,ctx,prf_counter) private(i) reduction(+:clockcycles)
	...
	#ifdef STRUCT_PRTL
		      // Free PRTL structure and get memory stats
		    	struct_free_PRTL();
	#endif
