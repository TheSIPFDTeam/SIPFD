...
int main (int argc, char **argv)
{
    ...
    (&ctx)->cores = (&context)->cores * ((&context)->deg + 1);
    table_size = (&context)->runtime[(&ctx)->c][0];
    (&ctx)->runtime[0][0] = (&context)->runtime[0][0];
    (&ctx)->runtime[1][0] = (&context)->runtime[1][0];
    ...
    PATHS = (fp_t *)malloc( (&ctx)->cores * sizeof(fp_t) );
